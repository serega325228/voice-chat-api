...
func (s *SignalingService) NewSignalingService(gRPCConn *grpc.ClientConn) {
	s.client = sessionv1.NewSessionClient(gRPCConn)
}

func (s *SignalingService) CreateSession(
	ctx context.Context,
...
func (s *SignalingService) JoinSession(
	ctx context.Context,
...
func (s *SignalingService) LeaveSession(
	ctx context.Context,
...
func (s *SignalingService) SignalPeer(
	stream sessionv1.Session_SignalPeerServer,
) error {
	const op = "SignalingService.SignalPeer"
...
func (s *SignalingService) toSignalMessage(peer *models.Peer, peerEvent *models.PeerEvent) *sessionv1.SignalMessage {
...
func (s *SignalingService) handleIncoming(
	ctx context.Context,
	peer *models.Peer,
	msg *sessionv1.SignalMessage,
) error {
	const op = "SignalingService.handleIncoming"

	switch x := msg.Payload.(type) {
	case *sessionv1.SignalMessage_RemoteOffer:
		return s.setRemoteOffer(ctx, peer, x.RemoteOffer)
	case *sessionv1.SignalMessage_RemoteIceCandidate:
		return s.setRemoteIceCandidate(ctx, peer, x.RemoteIceCandidate)
	case *sessionv1.SignalMessage_PeerClosed:
		return s.setPeerClosed(ctx, peer, x.PeerClosed)
	case *sessionv1.SignalMessage_ConnectionStateChanged:
		return s.setConnectionStateChanged(ctx, peer, x.ConnectionStateChanged)
	default:
		return fmt.Errorf("%s: %w", op, status.Error(codes.InvalidArgument, "unknown signal message payload"))
	}
}

func (s *SignalingService) setRemoteOffer(
	ctx context.Context,
	peer *models.Peer,
	offer *sessionv1.RemoteOffer,
) error {
	const op = "SignalingService.handleRemoteOffer"
...
func (s *SignalingService) setRemoteIceCandidate(
	ctx context.Context,
	peer *models.Peer,
	candidate *sessionv1.RemoteIceCandidate,
) error {
	const op = "SignalingService.handleRemoteIceCandidate"
...
func (s *SignalingService) setPeerClosed(
	ctx context.Context,
	peer *models.Peer,
	_ *sessionv1.PeerClosed,
) error {
	const op = "SignalingService.handlePeerClosed"
...
func (s *SignalingService) setConnectionStateChanged(
	ctx context.Context,
	peer *models.Peer,
